...
// ────────────────────────── FOOTER ──────────────────────────
function PageFooter() {
  const linkCls = "text-sm transition-colors hover:text-white" as const;
  const headingCls = "text-xs font-bold text-white uppercase tracking-[0.2em] mb-6" as const;
  const dimmed = { color: "rgba(255,255,255,0.55)" };
  return (
    <footer className="py-16 px-4 sm:px-6 lg:px-8" style={{ background: C.primaryDeep }}>
      <div className="max-w-[1200px] mx-auto">
        <div className="grid md:grid-cols-4 gap-12 mb-12">
          ...
          <div>
            <div className="mb-6">
              <img src="/api/aryo-logo" alt="Aryo Consulting Group" width={80} height={80} className="object-contain brightness-0 invert" />
            </div>
            <p className="text-sm leading-relaxed" style={dimmed}>
          ...
          {/* Services */}
          <div>
            <h4 className={headingCls}>Services</h4>
            <ul className="space-y-3">
              {[
                ...
                ["Talent & Organization", "/talent-organization"],
                ["Growth Strategy", "/growth-strategy"],
                ["Pitch Decks", "/pitch-decks"],
              ].map(([label, href]) => (
                <li key={href}><a href={href} className={linkCls} style={dimmed}>{label}</a></li>
              ))}
            </ul>
          </div>
          ...
          {/* Tools & Connect */}
          <div>
            <h4 className={headingCls}>Tools & Resources</h4>
            <ul className="space-y-3">
              {[
                ["PE Valuation Tool", "/tools/pe-valuation-tool"],
                ["Stablecoin Calculator", "/tools/stablecoin-calculator"],
                ["Website Analyzer", "/tools/website-analyzer"],
                ["Valuation Tool", "/valuation-tool"],
                ["AI Consultant", "/ai-consultant"],
                ["Pitch Deck", "/pitch-deck"],
                ["Q4 Hiring Report", "/reports/q4-hiring-abroad"],
              ].map(([label, href]) => (
                <li key={href}><a href={href} className={linkCls} style={dimmed}>{label}</a></li>
              ))}
            </ul>
            ...
            <div className="flex gap-4">
              <a href="https://www.linkedin.com/company/aryo-consulting/" target="_blank" rel="noopener noreferrer" aria-label="LinkedIn"
              ...
              <a href="https://www.instagram.com/aryoconsulting" target="_blank" rel="noopener noreferrer" aria-label="Instagram"
                className="w-10 h-10 flex items-center justify-center transition-colors hover:bg-white/10"
                style={{ border: "1px solid rgba(255,255,255,0.25)", color: "rgba(255,255,255,0.6)" }}>
        ...
        <div className="pt-8 flex flex-col md:flex-row justify-between items-center gap-4" style={{ borderTop: "1px solid rgba(255,255,255,0.1)" }}>
          ...
          <div className="flex gap-8 flex-wrap justify-center">
            {[["Privacy Policy", "/privacy-policy"], ["Terms of Service", "/terms-of-service"], ["Cookie Settings", "/cookie-settings"]].map(([label, href]) => (
              <a key={href} href={href} className="text-xs transition-colors hover:text-white" style={{ color: "rgba(255,255,255,0.4)" }}>{label}</a>
            ))}
          </div>
        </div>
      </div>
